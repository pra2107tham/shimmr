...
Deno.serve(handler(async (req) => {
  ...
  const { data: install, error: lookupErr } = await db
    .from("installs")
    .select("id, user_id, revoked_at, users(org_id)")
  ...
  const orgID = (install.users as { org_id: string } | null)?.org_id;
  if (!orgID) throw new HttpError(500, "install is not attached to an organisation");
